...
- (void)testGrowingModuleManager {
    ...
    NSMutableArray *moduleInfos = [[GrowingModuleManager sharedInstance] performSelector:@selector(growingModuleInfos)];
    BOOL isFind = NO;
    ...
    NSMutableArray *moduleInfos2 = [[GrowingModuleManager sharedInstance] performSelector:@selector(growingModuleInfos)];
    BOOL isFind2 = NO;
    for (NSDictionary *dict in moduleInfos2) {
        if ([dict[@"moduleClass"] isEqualToString:NSStringFromClass([GrowingModuleTest class])]) {
            isFind2 = YES;
            break;
        }
