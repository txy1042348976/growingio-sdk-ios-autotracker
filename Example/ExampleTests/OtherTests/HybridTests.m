...
- (void)testWebCircle {
    ...
    NSMutableDictionary *dict = [[[GrowingWebCircle alloc] init] performSelector:@selector(dictFromPage:xPath:)
                                                                      withObject:current
                                                                      withObject:page.path];
    ...
    [[[GrowingWebCircle alloc] init] performSelector:@selector(runWithCircle:readyBlock:finishBlock:)
                                          withObject:[NSURL URLWithString:@"ws://testws"]
                                          withObject:nil];
    ...
    NSURL *urltest = [[NSURL alloc] initWithString:
                                        @"http://testxxx.growingio.com/"
    ...
    [[[GrowingWebCircle alloc] init] performSelector:@selector(webSocket:didReceiveMessage:)
                                          withObject:nil
                                          withObject:messageready];
    [[[GrowingWebCircle alloc] init] performSelector:@selector(webSocket:didReceiveMessage:)
                                          withObject:nil
                                          withObject:messageincompatible];

    //   [[[GrowingWebCircle alloc]init] performSelector:@selector(webSocket:didReceiveMessage:)withObject:nil
    //   withObject:messagequit];
    ...
    //    [[realClazz performSelector:@selector(sharedInstance)]
    //    performSelector:@selector(sendWebcircleWithType:)withObject:@"eventType"];;
}

- (void)testGrowingHandleUrl {
    NSURL *url = [NSURL URLWithString:
                            @"growing.9683a369c615f77d://growing/oauth2/"
                            @"token?messageId=GPnmM2RY&gtouchType=preview&msgType=popupWindow&"];
...
- (void)testGrowingUserDefaults {
    ...
    //((_refreshToken) equal to (@"testToken")) failed: ("{length = 8, bytes = 0x0000000000000000}") is not equal to
    //("{length = 8, bytes = 0xe82abb2801000000}")
...
- (void)testGrowingHybridBridgeProvider {
    ...
    builder = [[GrowingHybridBridgeProvider sharedInstance] performSelector:@selector(transformViewElementBuilder:)
                                                                 withObject:evetDataDict];
    XCTAssertNotNil(builder);
}
